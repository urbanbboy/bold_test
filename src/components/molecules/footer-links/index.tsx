...
export const FooterLinks = () => {
    ...
    items: LinkProps[];
      ...
      {
          title: t("titles.about") || "О компании",
          items: [
      ...
      },
      {
          ...
          items: [
              ...
              {
                  title: t("servicesLinks.videoProduction"),
                  href: "/services/video-production",
              },
              {
                  title: t("servicesLinks.webDevelopment"),
                  href: "/services/site-creating",
              },
              {
                  title: t("servicesLinks.marketingSupport"),
                  href: "/services/marketing-support",
              },
              {
                  title: t("servicesLinks.automationAnalytics"),
                  href: "/services/crm",
              },
              {
                  title: t("servicesLinks.printing"),
                  href: "/services/operative-print",
              },
          ],
      },
      {
          title: t("titles.we") || "Мы есть",
          items: [
              {
                  href: "https://www.instagram.com/boldbrands.international",
                  icon: <InstagramIcon />,
                  label: "Instagram",
              },
              {
                  href: "https://www.facebook.com/boldbrands.kg",
                  icon: <FacebookIcon />,
                  label: "Facebook",
              },
              {
                  href: "https://wa.me/996999992244",
                  icon: <WhatsAppIcon />,
                  label: "WhatsApp",
              },
          ],
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-8 lg:gap-y-10 gap-x-3">
            {links.map(({ title, items }, idx) => (
                <div key={idx} className="flex flex-col gap-y-3">
                    ...
                    <ul
                        className={cn(
                            "text-lg text-gray2",
                            items[0].icon ? "flex gap-x-3" : "space-y-3"
                        )}
                    >
                        {items.map((item) =>
                            item.icon ? (
            ...
            {contacts.map((contact, idx) => (
                <ul key={idx} className="flex flex-col gap-y-3">
                    <ContactItem
                        label={contact.label}
                        title={contact.title}
                        href={contact.href}
...
const SocialItem = ({ href, icon, label }: LinkProps) => (
    <li className="flex items-center gap-x-2">
        <Link
            className="hover:text-accent rounded-full hover:shadow-md transition-all duration-300"
            href={href || ""}
            target="_blank"
            aria-label={label}
        >
