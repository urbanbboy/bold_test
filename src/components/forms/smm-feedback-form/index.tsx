...
export const SmmFeedbackForm = () => {
    ...
    const handleNextStep = () => {
        if (selectedBusinessTypes.length > 0 && selectedServices.length > 0 && selectedPromotion.length > 0) {
    ...
    const onSubmit = (data: z.infer<typeof SmmFeedbackFormSchema>) => {
        if (!selectedServices.length || !selectedBusinessTypes.length || !selectedPromotion.length) {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        <div className="space-y-5">
                            <Tabs value={tabValue} onValueChange={(value) => {
                                ...
                                else if (selectedBusinessTypes.length > 0 && selectedServices.length > 0) {
                                    setTabValue(value);
                                } else {
                                    toast.error("Выберите хотя бы один вариант в каждом поле");
                                }
